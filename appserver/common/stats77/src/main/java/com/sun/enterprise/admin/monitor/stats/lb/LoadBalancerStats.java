...
public class LoadBalancerStats implements LoadBalancerStatsInterface, CommonBean {
	...
	public LoadBalancerStats(LoadBalancerStats source) {
		for (java.util.Iterator it = source._ClusterStats.iterator(); 
			it.hasNext(); ) {
			_ClusterStats.add(new ClusterStats((ClusterStats)it.next()));
		}
	}
	...
	public void writeNode(java.io.Writer out, String nodeName, String indent) throws java.io.IOException {
		...
		for (java.util.Iterator it = _ClusterStats.iterator(); 
			it.hasNext(); ) {
			ClusterStats element = (ClusterStats)it.next();
			if (element != null) {
	...
	public void readNode(org.w3c.dom.Node node) {
		...
		for (int i = 0, size = children.getLength(); i < size; ++i) {
			...
			if (childNodeName == "cluster-stats") {
				ClusterStats aClusterStats = new ClusterStats();
	...
	public static class ValidateException extends Exception {
		...
		public CommonBean getFailedBean() {return failedBean;}
	}
	...
	public void validate() throws ValidateException {
		...
		for (int _index = 0; _index < sizeClusterStats(); ++_index) {
			ClusterStats element = getClusterStats(_index);
			if (element != null) {
				element.validate();
			}
		}
	...
	public boolean equals(Object o) {
		if (o == this)
			return true;
		if (!(o instanceof LoadBalancerStats))
			return false;
		LoadBalancerStats inst = (LoadBalancerStats) o;
		...
		for (java.util.Iterator it = _ClusterStats.iterator(), it2 = inst._ClusterStats.iterator(); 
			...
			ClusterStats element = (ClusterStats)it.next();
			ClusterStats element2 = (ClusterStats)it2.next();
