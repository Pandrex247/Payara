...
class ConfigFile extends AuthConfig {
    ...
    public ClientAuthContext getClientAuthContext(String intercept, String id, AuthPolicy requestPolicy, AuthPolicy responsePolicy,
        ...
        Entry[] entries = getEntries(intercept, id, requestPolicy, responsePolicy, CLIENT);
    ...
    public ServerAuthContext getServerAuthContext(String intercept, String id, AuthPolicy requestPolicy, AuthPolicy responsePolicy,
        ...
        Entry[] entries = getEntries(intercept, id, requestPolicy, responsePolicy, SERVER);
        if (entries == null || entries.length == 0) {
            return null;
        }
    ...
    private Entry[] getEntries(String intercept, String id, AuthPolicy requestPolicy, AuthPolicy responsePolicy, String type) {
        ...
        for (int i = 0; i < entries.length; i++) {
            ...
            entries[i] = new Entry(reqP, respP, idEntry.getModuleClassName(),
    ...
    static class Entry extends AppConfigurationEntry {
         ...
         */
        Entry(AuthPolicy requestPolicy, AuthPolicy responsePolicy, String moduleClass, LoginModuleControlFlag flag,
                Map options) {
